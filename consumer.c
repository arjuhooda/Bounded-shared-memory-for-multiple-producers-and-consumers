...
int main()
{
		...
		while(1)
		{
			...
			semaphores.sem_num = 2;			// full semaphore
			semaphores.sem_op = -1; 		// decrement of full sem by 1
			semop(semaphore_id, &semaphores, 1);	// wait(full)
			semaphores.sem_num = 0;			// mutex
			semaphores.sem_op = -1;			// decrement of mutex by 1
			semop(semaphore_id, &semaphores, 1);	// wait(mutex)
			
			// consumer
			printf("Press any key to consume...");
			if (getchar()== '0')
				break;
			i = atoi(&pointer[BUFF_SIZE+1]);
			cons = pointer[i];
			printf("Consumer is consuming a character %c at %d\n", cons, i);
			i = (i+1)%BUFF_SIZE;
			pointer[BUFF_SIZE+1] = i+'0';
			printf("Consumed\n");
			...
			semaphores.sem_num = 1;			// empty semaphore
			semaphores.sem_op = 1;			// increment of empty sem by 1
			semop(semaphore_id, &semaphores, 1);	// signal(empty)
